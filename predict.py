...
# ---------------- CONFIGURAR AQUI ----------------
WEEKLY_PARQUET = "weekly_full.parquet"   # seu arquivo com histórico semanal
MODEL_DIR      = "models"
...
def build_features_for_target_week_vectorized(hist, pairs, target_week, lags=LAGS, roll_windows=ROLL_WINDOWS):
    ...
    """
    target_week = pd.to_datetime(target_week).normalize()
    Xw = pairs.copy()
    ...
    for l in lags:
        wk = (target_week - pd.Timedelta(weeks=l)).normalize()
    ...
    for w in roll_windows:
        weeks = [(target_week - pd.Timedelta(weeks=k)).normalize() for k in range(1, w+1)]
...
weekly_full['week_start'] = pd.to_datetime(weekly_full['week_start']).dt.normalize()
...
train_end = (test_start - pd.Timedelta(weeks=1)).normalize()
...
for i, tgt in enumerate(test_weeks):
    ...
    # prever (compatível com Booster e sklearn wrapper)
    if isinstance(model, lgb.Booster):
        yhat = model.predict(X_pred)   # ordenação das colunas não importa para Booster.predict se X é numpy ou df com colunas na mesma ordem
    else:
